...
public class PayloadBuilder {
    ...
    public static String buildSessionReport(SessionSnapshot s) {
        ...
        embed.addProperty("color", isError ? COLOR_ERROR
                : "disconnect".equals(s.reason) ? COLOR_DISCONNECT
                  : COLOR_OK);
        ...
        addField(fields, "FPS (avg/min/max)",
                s.fpsAvg + " / " + s.fpsMin + " / " + s.fpsMax, true);
        ...
        if (isError && s.errorStacktrace != null) {
            ...
            if (trace.length() > 3800) {
                trace = trace.substring(0, 3800) + "\n... (truncated)";
            }
